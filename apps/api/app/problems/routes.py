...
async def list_curated_problems(session: Annotated[AsyncSession, Depends(get_session)]) -> list[CuratedCatalogItem]:
...
async def curated_problem_detail(problem_version_id: UUID, language: Literal["cpp", "python", "java"], session: Annotated[AsyncSession, Depends(get_session)]) -> CandidateProblemDetail:
    try:
    ...
    language_definition = schema["languages"][language]
    return CandidateProblemDetail(
        **_catalog_item(version).model_dump(),
        statement=version.statement,
        constraints=list(version.constraints_json["items"]),
        examples=list(version.examples_json),
        display_signature=language_definition["display_signature"],
        starter_code=language_definition["starter_code"],
        argument_schema=list(execution["arguments"]),
...
def _catalog_item(version: object) -> CuratedCatalogItem:
    ...
    return CuratedCatalogItem(
        ...
        catalog_order=int(version.io_schema_json["catalog_order"]),
    )
