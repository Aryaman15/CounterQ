...
ASSESSMENT_EVALUATOR_POLICY_VERSION = "v1"
ASSESSMENT_INPUT_CONTRACT_VERSION = "assessment-input.v1"
...
finding must cite source aliases from the allowlist and canonical concept and
SkillDimension keys from their allowlists. Never invent a target or source.
...
and transient slips are not meaningful Breakpoints. Independent correction can
support positive or mixed debugging/correctness Evidence while preserving the
earlier observed mistake.
...
polarity. CONTRADICTED may identify later positive/mixed evidence against the
same target. RESOLUTION_SUPPORT is conservative and does not itself resolve a
Breakpoint. Never author database status, Evidence validation, Mastery, or
